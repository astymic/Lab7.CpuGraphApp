...
namespace Lab7.CpuMonitoringLibrary
{
    public class CpuDataProvider : ICpuDataProvider
    {
        ...
        public int HistoryCapacitySeconds { get; }

        /// <summary>
        /// Initializes a new instance of the <see cref="CpuDataProvider"/> class.
        /// </summary>
        /// <param name="historySecondsToStore">The number of seconds of CPU usage history to store. Defaults to 60.</param>
        public CpuDataProvider(int historySecondsToStore = 60)
        {
            if (historySecondsToStore <= 0)
            {
                throw new ArgumentOutOfRangeException(nameof(historySecondsToStore), "History capacity must be greater than zero.");
            }

            HistoryCapacitySeconds = historySecondsToStore;
            _maxHistoryCount = historySecondsToStore; // Assuming 1 data point per second

            // Initialize history with default values
            _cpuUsageHistory = new Queue<float>(Enumerable.Repeat(0.0f, _maxHistoryCount));
        }
        ...
        public void StartMonitoring()
        {
            ...
            _updateTimer = new System.Timers.Timer(1000); // 1 second interval
        ...
        private void UpdateCpuData()
        {
            ...
            lock (_historyLock)
            {
                if (_cpuUsageHistory.Count >= _maxHistoryCount)
                {
